...
# COST SAVINGS: Using FREE stock videos instead of expensive Runway AI
# Saves $6,000/month! Get free API key: https://www.pexels.com/api/
...
class VideoAutomation:
    ...
    def generate_and_upload_shorts(self):
        ...
        try:
            ...
            logger.info("Step 6: Selecting background music...")
            background_music = None
            music_mood = script_data.get('background_music', 'upbeat and energetic')
            background_music = self.music_selector.get_music_for_mood(music_mood)
            if background_music:
                logger.info(f"Selected background music: {Path(background_music).name}")
            else:
                logger.warning("No background music available - continuing without BGM")
            ...
            logger.info("Step 9: Uploading to Instagram...")
    ...
    def generate_and_upload_short_video(self):
        ...
        try:
            ...
            logger.info("Step 6: Selecting background music...")
            background_music = None
            music_mood = script_data.get('background_music', 'upbeat and engaging')
            background_music = self.music_selector.get_music_for_mood(music_mood)
            if background_music:
                logger.info(f"Selected background music: {Path(background_music).name}")
            else:
