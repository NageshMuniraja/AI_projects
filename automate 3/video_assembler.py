...
class VideoAssembler:
    ...
    def _mix_audio_with_bgm(self, voice_audio, bgm_path, target_duration):
        """Mix voice audio with background music"""
        try:
            if not bgm_path or not Path(bgm_path).exists():
                logger.warning(f"Background music not found: {bgm_path}")
                return voice_audio
            ...
            if bgm.duration < target_duration:
                # Loop the BGM
                loops_needed = int(target_duration / bgm.duration) + 1
                bgm = concatenate_videoclips([bgm] * loops_needed)
            ...
            # Mix voice and BGM
            mixed_audio = CompositeAudioClip([voice_audio, bgm])
            
            logger.info(f"Mixed voice audio with background music at {self.bgm_volume*100}% volume")
            return mixed_audio
            
        except Exception as e:
            logger.error(f"Error mixing audio with BGM: {e}")
            return voice_audio
    ...
    def assemble_shorts_video(
        ...
        Args:
            ...
            audio_path: Path to audio file (voice narration)
        ...
        try:
            ...
            if background_music_path:
                audio = self._mix_audio_with_bgm(audio, background_music_path, actual_duration)
    ...
    def assemble_short_video(
        ...
        """
        Assemble a 3-minute entertaining video
    ...
    def _trim_clips_to_duration(self, clips, target_duration):
        ...
        for clip in clips:
            ...
            if clip.duration <= remaining_duration:
                ...
                # Trim the last clip
                trimmed = clip.subclipped(0, remaining_duration)
    ...
    def _add_text_overlays(self, video_clip, text_overlays):
        ...
        for overlay in text_overlays:
            ...
            try:
                # Render text to image using Pillow to avoid ImageMagick/TextClip
                from PIL import Image, ImageDraw, ImageFont
                from moviepy import ImageClip

                w = video_clip.w - 100
                # Create image with transparent background
                # Estimate height by simple linewrap
                font_size = 60
                try:
                ...
                words = text.split()
                line = ""
                img_tmp = Image.new("RGBA", (w, 2000), (0, 0, 0, 0))
                draw_tmp = ImageDraw.Draw(img_tmp)
                for word in words:
                    test = (line + " " + word).strip()
                    bbox = draw_tmp.textbbox((0, 0), test, font=font)
                    if bbox[2] > w and line:
                        lines.append(line)
                        line = word
                    else:
                        line = test
                ...
                line_h = draw_tmp.textbbox((0, 0), "Ay", font=font)[3]
                ...
                # Save temp image
                tmp_path = Path(Config.TEMP_DIR) / f"overlay_{int(time.time()*1000)}.png"
                tmp_path.parent.mkdir(parents=True, exist_ok=True)
                img.save(tmp_path)
                temp_images.append(tmp_path)

                # Create ImageClip
                img_clip = ImageClip(str(tmp_path)).set_duration(duration)

                # Positioning
                if position == 'bottom':
                    img_clip = img_clip.set_position(('center', video_clip.h - img_h - 50))
                elif position == 'top':
                    img_clip = img_clip.set_position(('center', 50))
                else:
        ...
        composite = CompositeVideoClip(composite_clips)

        # Clean up temp images when finished (MoviePy keeps them while writing)
        def _cleanup():
        ...
        # Attach cleanup to composite (no direct hook; attempt best-effort after a short delay)
        try:
            _cleanup()
        except Exception:
            pass
    ...
    def _create_text_screen(self, text, duration, size, bg_color=(100, 150, 255)):
        """Create a simple text screen using Pillow and ImageClip to avoid ImageMagick/TextClip."""
        try:
            from PIL import Image, ImageDraw, ImageFont
            ...
            lines = []
            line = ""
            img_tmp = Image.new("RGBA", (text_w, 2000), (0, 0, 0, 0))
            draw_tmp = ImageDraw.Draw(img_tmp)
            for word in words:
                test = (line + " " + word).strip()
                bbox = draw_tmp.textbbox((0, 0), test, font=font)
                if bbox[2] > text_w and line:
                    lines.append(line)
                    line = word
                else:
                    line = test
            ...
            line_h = draw_tmp.textbbox((0, 0), "Ay", font=font)[3]
            img_h = line_h * len(lines) + 40
            img = Image.new("RGBA", (text_w, img_h), (0, 0, 0, 0))
            draw = ImageDraw.Draw(img)
            ...
            tmp_path = Path(Config.TEMP_DIR) / f"screen_{int(time.time()*1000)}.png"
            tmp_path.parent.mkdir(parents=True, exist_ok=True)
            img.save(tmp_path)
            ...
            screen = CompositeVideoClip([bg, img_clip])
            ...
            logger.error(f"Error creating text screen: {e}")
            return None
