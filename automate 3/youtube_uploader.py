...
class YouTubeUploader:
    ...
    def _authenticate(self):
        ...
        # Check if credentials file exists and what format it is
        if credentials_file.exists():
            import json
            with open(credentials_file, 'r') as f:
                creds_data = json.load(f)
            
            # Check if it's already a token format (has 'token' field)
            if 'token' in creds_data:
                logger.info("Using token-based credentials from youtube_credentials.json")
        ...
        if not self.credentials or not self.credentials.valid:
            if self.credentials and self.credentials.expired and self.credentials.refresh_token:
                ...
                # OAuth flow for standard client secrets format
